...
export function useTrackedLots(user) {
  ...
  const add = useCallback(async (rawUrl) => {
    ...
    if (!supported.some(re => re.test(url))) {
      return { error: 'Only eBay item URLs are supported here. For auction-house lots (Antiquorum / Christie\'s / Sotheby\'s / Phillips / Watches of Knightsbridge / Marteau & Co) heart them from the main feed; they\'re scraped automatically.' };
    }
