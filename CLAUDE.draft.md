# Working with this repo — DRAFT (Stage 2 reset of CLAUDE.md)

> **This is a proposal.** The live `CLAUDE.md` is untouched. Target:
> conventions only, no changelog / PR tags / architecture description.
> Aiming ~4,000 words (hard ceiling 6,000). `«confirm?»` marks a cut or
> call I want your eyes on. `→ code comment` marks a single-site rule
> slated to move into the file it governs (Stage E).
...
| **LEARNING.md** | Mark's builder-growth log | Updated at close; never folded here |
...
- One fact, one home: **SHIPPED** (ship events, 2 lines) · **ROADMAP**
  ...
  **DESIGN_SYSTEM** (token/component changes) · **CLAUDE.md** (a durable
  ...
  tags).
- Update **LEARNING.md** (capability snapshot / concept log / growth edges).
...
- **Image-proxy = THREE-place lockstep** when adding a hot-link-protected
  ...
  in `imgSrc()`; Sotheby's brightspot is hash-signed — don't retry URL rewrites. `→ code comment`
- **`is_excluded_title`** filters only pocket watches / clocks / loose dials;
...
- **Phillips: never fetch lot detail from CI** (WAF 403s after ~7 requests).
  ...
  comes from the rendered "Sold For" panel, not JSON-LD (that's the low estimate). `→ code comment`
- **Comprehensive auction houses (5):** Antiquorum (use `live.antiquorum.swiss`
...
- **App.js hook ordering.** Never add `useState`/`useMemo`/`useCallback`
  *after* the `if (loading)` / `if (loadError)` early returns — it triggers
  ...
  must not reference state declared later in the function (TDZ). `→ partly code comment`
...
- **Portals + theme.** Card ⋯ menu, overlays, toasts render via
  ...
  portals regress (no background / iOS Times serif). `→ code comment`
- **Confirm dialogs:** `await confirm({...})` from `./ConfirmModal`. Never
...
## Single-site gotcha index (filled in Stage E)
