...
- **Reported:** 2026-08-30 (Mark — email alerts) · **Type:** Scrape defect (upstream) · **Severity:** 2 · **Surface:** `watchcenter_scraper.py` · **Status:** Open — root-caused, needs Mark's call.
...
- **Next:** Mark to open `watchcenter.ch` in a browser — if the storefront is up but `/wp-json/wc/store/v1/products` 503s, the REST API has been turned off and the source needs a different ingestion path (or retiring); if the whole site is down, wait it out. Until then it will page every run — consider a per-source snooze on the B-60 gate rather than living with a permanently red scrape.

### B-81 — Watches of Lancashire: 403 even with curl_cffi Chrome impersonation (11 consecutive misses)
- **Reported:** 2026-08-30 (Mark — email alerts) · **Type:** Scrape defect (upstream anti-bot) · **Severity:** 2 · **Surface:** `watchesoflancashire_scraper.py` · **Status:** Open — root-caused, fix not yet attempted.
...
- **Hypothesis (untried, cheap):** the scraper hits the API endpoint cold. Luna Royster warms its session with a `SESSION.get(BASE + "/")` first and does not get 403'd; Wordfence/Cloudflare commonly issue the clearance cookie on the HTML page and 403 direct `/wp-json/` hits that never got one. Try a warm-up GET + cookie reuse before escalating to the residential-host pattern (B-25) that Bonhams uses.
