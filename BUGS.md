...
### B-97 — Maunder Watches + Watches of Knightsbridge both stopped returning JSON (3 consecutive misses, paged 2026-09-07)
- **Reported:** 2026-09-07 (Mark — "Scrape listings: All jobs have failed" email; run 34151928856) · **Type:** Scrape defect (upstream) · **Severity:** 2 · **Surface:** `maunderwatches_scraper.py` + `watchesofknightsbridge_scraper.py` · **Status:** Open — root-caused from the CI log, fix not attempted.
- **Detail:** Both sources missed 3 consecutive runs, crossing the B-60 gate, which correctly failed the job. Neither is a code change on our side — both are WooCommerce **Store API** endpoints that stopped returning JSON, within the same window:
  - **Maunder:** `RuntimeError: offset 0 failed after 3 attempts: HTTP 202`. A 202 on a product API is the anti-bot "accepted, come back later" interstitial CLAUDE.md already names for the auction houses. The retry ladder can't help: all three attempts get 202.
  - **Knightsbridge:** `JSONDecodeError` out of `r.json()` at `watchesofknightsbridge_scraper.py:67` — the endpoint returned a body that isn't JSON (HTML interstitial), despite already using `curl_cffi impersonate="chrome"`.
- **Why it matters little right now:** `merge.py` keeps the prior CSVs, so the app still serves both dealers (WoK 61, Maunder 72). They are going STALE, not disappearing. The cost is the paging, plus drift.
- **Two of them at once is the clue.** Same platform, same week, same failure family — likely one bot-protection product rolled out by a shared host rather than two dealers independently deciding to block us. Worth one `source-probe` per host (now with `show_body=true`) to see what the interstitial actually is before writing code: a JS challenge can't be fixed by TLS impersonation (B-81), a plain UA/fingerprint check can.
- **Fix options, in order of cost:** (a) probe both, confirm the interstitial; (b) if it is a fingerprint check, `curl_cffi` impersonation on Maunder (it still uses plain `requests` via `SESSION`) and a fresher chrome version on WoK; (c) if it is a JS challenge, these join the residential-agent set like Bonhams; (d) if this drags on, a dated `data/scrape_health_snooze.json` entry to stop the paging while it is out of our hands — the B-80 pattern.
