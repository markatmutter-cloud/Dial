...
- **Reported:** 2026-05-31 (Mark, screenshot — two Tropical Watch "1996/1997 Rolex Explorer 14270" listings showing model "Submariner") · **Type:** Data quality / categorisation · **Severity:** 2 · **Surface:** `public/watch_references_index.json` (+ `reference_index_match.py`, `merge.py`) · **Status:** Open. **Root cause (confirmed):** the curated index lists reference **`14270` under `Rolex › Submariner`** — but 14270 is the Rolex **Explorer**. The matcher faithfully stamps model_line "Submariner" onto those Explorer listings. **Fix:** correct the index (remove 14270 from Submariner refs; it belongs under Explorer). NB the matcher only fills when EMPTY, so already-tagged listings won't self-correct on the index fix alone — need a re-tag/force pass (or wait for re-enrichment). Also check why **brand isn't resolving to "Rolex"** on these (title clearly says Rolex — possible Tropical Watch scraper brand gap or a `FORCE_OTHER_BRANDS`/alias issue). Cousin of B-26 (the "002" ref collision). Quick data fix + a re-tag.
