...
- **Reported:** 2026-05-27 (Mark's wife, real user test + Mark) · **Type:** Feature gap / save-tangle · **Severity:** 2 · **Surface:** article page (`EditorialView`/article detail) + `ReferencePage` · **Status:** ✅ **RESOLVED 2026-06-02.** Articles are savable via `articleAsListing` (heart + ⋯ on article cards); reference-guide saving shipped #731 (`referenceAsListing`, save on `ReferencePage`) and the guide CARDS got heart + ⋯ (save / add-to-list / share / Ask Lumé) #757. You can now save watches, articles AND reference guides from where you read them. Original: only listings could be hearted; articles and reference guides had no heart / "add to list" affordance from where you encounter them. But the dossier (Phase 1) is meant to hold articles + reference guides + watches, so you must be able to **save them at the point of reading**. Add a heart / add-to-list control on the article detail + reference node pages, routing through the existing collection-picker (`openCollectionPicker`) + the `listing_snapshot.kind = 'article'` / reference-node patterns. Different files from B-08 → its own branch.

### B-39 — AI concierge bubble: UI polish defects (first real test)
- **Reported:** 2026-05-29 (Mark, first live test of the Epic 9 concierge) · **Type:** Usability/polish · **Severity:** 2 · **Surface:** `src/components/ChatBubbleHost.js` · **Status:** ✅ **RESOLVED** (shipped across the Lumé v1 polish #680–#682 + follow-ups; verified in code 2026-06-02). All six landed: input text forced to `--text1` (+ `WebkitTextFillColor`), a visible `×` close, a name in the header, an animated "Lumé is thinking" indicator, markdown bold/line-breaks rendered, and bare URLs auto-linkified (new tab). Original defects from the first session:
...
- **Reported:** 2026-05-29 (Mark) · **Type:** Model/prompt tuning · **Severity:** 2 · **Surface:** `SYSTEM_PROMPT` in `api/chat.js` / `public/lume_system_prompt.txt` · **Status:** ✅ **RESOLVED** (prompt hardening #709/#713/#715; verified 2026-06-02). The prompt now enforces both: facts ONLY from tool calls + "accuracy beats colour" (omit rather than approximate an era), and a "NEVER DEAD-END — always suggest a next step from a real capability" section. Two principles from the first test:
...
- **Reported:** 2026-05-31 (Mark, screenshot: Lists ▸ Saved with `Source · 1` = Belmont Watches active, narrowing to 4) · **Type:** Usability / cross-surface consistency · **Severity:** 2 · **Surface:** the shared listings filter bar (Source / Brand / Model / search / price / date + source chips) used across **Watches** and **Lists** · **Status:** ✅ **Fixed PR#729** — the shared `ActiveFiltersStrip` (chips + right-aligned "Clear all") already lived on Watches + the Searches sub-tab; the gap was that the Lists/Saved surface never received it. The Lists redesign (Phase 1) gives the new **Hearted** sub-tab the standard filter bar + strip, so Lists now inherits the same Clear-all as Watches (one shared component, per Mark's framing). The "Save search" half is the existing heart-by-search-bar affordance. **Detail:** when filters are active there's no clear/reset-all control — the only signal is the subtle `Source · 1` pill, and no one-tap way to clear. **Mark's framing (the real point):** a filterable surface should ALWAYS expose a "Clear filters" feature — make it a **shared mechanism** (one component / single source of truth, "a standard library") so every filter surface inherits it, not a per-surface patch. **Fix:** add a "Clear all" control to the shared filter bar that appears whenever any filter/sort deviates from default and resets brand/model/source/search/price/date — wired once in the shared chrome so Watches + Lists both get it (cross-surface consistency = shared abstraction, per CLAUDE.md).

### B-56 — About / "Nexus" page redesign (3 voices + Lumé-led guided intro) + brand/voice/copy review
- **Reported:** 2026-06-02 (Mark) · **Type:** Design + content (voice-heavy) thread → plan-mode · **Severity:** 2 · **Surface:** `src/components/AboutModal.js` (currently a modal — What-you-can-do / How-to-use / Why-I-built-this / Get-in-touch), `App.js` (`aboutModalOpen`), `BRAND.md` · **Status:** OPEN — **scoped, deferred** (NOT the immediate next session; Mark wants it on the backlog to resurface). Build spec in memory [[project_about_page_sections]].
- **Scope (the plan):**
...
### B-54 — Rolex Explorer 14270 mis-tagged as "Submariner" (bad reference-index data)
- **Reported:** 2026-05-31 (Mark, screenshot — two Tropical Watch "1996/1997 Rolex Explorer 14270" listings showing model "Submariner") · **Type:** Data quality / categorisation · **Severity:** 2 · **Surface:** `public/watch_references_index.json` (+ `reference_index_match.py`, `merge.py`) · **Status:** Fixed (branch `b54-reference-index-mislabels`). **Root cause (deeper than reported):** TWO bugs. (1) The main `Explorer / Explorer II` model line put its refs under `**Refs (Explorer)**` / `**Refs (Explorer II)**` — labels `parse_index` doesn't recognise (it only honours the literal `**Refs**:` key and *overwrites* on a second one), so the entire Explorer ref set was **orphaned** (`refs: []`). `14270` only matched at all because someone parked it under `Submariner` with an `(n/a—Explorer)` note as a workaround — the wrong line. (2) `match_against_index` was brand-blind and returned the *first* candidate token's hit, so a leading **year** that collides with another brand's ref (`1991` → Cartier Panthère ref 1991) won first, then merge.py's cross-pollination guard rejected the whole hit → `model_line` came back empty for "1991 Rolex Explorer 14270". (Brand *was* resolving to Rolex correctly — the reported "Submariner" came from year-dependent behaviour: years absent from the index fell through to the mis-filed 14270→Submariner; years that are another brand's ref nulled it.) **Fix:** merged Explorer refs onto one `**Refs**:` line (also indexes the long-orphaned 6098/124270/224270/216570/… set) + removed 14270 from Submariner + made `match_against_index` brand-aware (skip cross-brand token collisions, keep scanning for a same-brand ref). `model_line` isn't persisted in state.json, so the next scrape re-tags all 29 affected rows (Falco/MVV/Bulang/Huntington/Tropical) to Explorer automatically — verified locally. Regression test `tests/test_reference_match.py`. Cousin of B-26. Spun off B-57 for the same-brand collision sweep.

### B-57 — Same-brand reference collisions in the curated index (model_line ambiguity)
- **Reported:** 2026-06-02 (Claude, sweep while fixing B-54) · **Type:** Data quality / categorisation · **Severity:** 3 · **Surface:** `docs/watch_references.md` · **Status:** Open — needs a domain pass with Mark. **Detail:** a sweep found 77 refs mapping to >1 (brand, model_line). The cross-pollination guard already neutralises **cross-brand** collisions in practice (a Rolex listing can't match a TAG Heuer entry), and most same-brand ones are intentional *layered* entries (a ref under both `Carrera (vintage)` and `Carrera (additions)` — same watch). The genuinely-risky residue is **same-brand, different-family** refs, where the matcher just takes whichever the index ordered first (exactly how 14270 went wrong). Shortlist to adjudicate: Patek `5236P` (Calatrava vs Perpetual Calendar), Cartier `WJTA0001` (Crash vs Tank Américaine), IWC `IW5004` (Big Pilot vs Portugieser), UG `222102`/`22409` (Aero-Compax vs Tri-/Uni-Compax), Lange `216.026` (Richard Lange vs Saxonia) + `233.026`, Patek `5004` (Chronograph vs Grand Complications), Rolex `16264`/`17013`/`17014` (Datejust vs Turn-O-Graph/Oysterquartz). **NB** the broader list also touched Heuer `73643NT` (Camaro vs Carrera) — handle per Mark's standing "don't generalise Camaro→Carrera" note; do NOT auto-edit. **Action:** review the shortlist with Mark; correct genuine mis-files in the source `.md`; leave layered same-family entries alone.

### B-44 — synthesise-saved-nodes workflow commits to main with NO CI gate
- **Reported:** 2026-05-30 (bit us live) · **Type:** CI / process reliability · **Severity:** 2 · **Surface:** `.github/workflows/synthesise-saved-nodes.yml` · **Status:** Resolved 2026-06-02 — **hybrid of (a)+(b)**: the workflow now runs `pytest` + `jest` on the generated outputs (the gate — fails the run, nothing committed, if red) AND opens a **PR** instead of pushing to main (review checkpoint + branch discipline). Both were needed: a PR opened by the default `GITHUB_TOKEN` does NOT trigger `tests.yml` (GitHub suppresses workflow-triggered `pull_request` events), so the PR alone wouldn't run CI — the in-workflow test run is what actually closes the hole, without needing a PAT secret.
- **Detail:** the saved-node fan-out used to commit `reference_synthesis_*.json` etc. **straight to main without running tests**. On 2026-05-30 the new `reference_synthesis_rolex-datejust.json` invalidated two tests (jest `getReference` "Datejust → empty synthesis"; pytest `test_derive_nodes` "datejust in nodes") and **turned main red with no warning** — only surfaced when the next PR (#704) failed CI.
...
- **Reported:** 2026-05-24 · **Source:** `audit:2026-05-24` · **Severity:** 3 (footgun) · **Surface:** auction scrapers · **Status:** Resolved 2026-06-02 — renamed `auctionlots_scraper.py` → `tracked_lots_scraper.py`; updated the `manual_archive_scraper.py` import + `scrape-auctions.yml` + docs. No longer one underscore from `auction_lots_scraper.py`.
- **Detail:** `auction_lots_scraper.py` (catalog walker) and `auctionlots_scraper.py` (tracked-URL tracker) differed by one underscore and both run in the same workflow — easy to edit the wrong one. Flagged independently by 3 auditors.
...
- **Reported:** 2026-05-26 · **Severity:** 2 (a source stops updating; operability) · **Surface:** `tropicalwatch_scraper.py`, `.github/workflows/scrape-listings.yml`, `notify-scrape-failure.yml` · **Status:** ✅ **RESOLVED** (confirmed 2026-06-02) — the **direct `requests` scraper shipped** and `tropicalwatch_scraper.py` no longer touches Browse AI; `data/tropicalwatch.csv` updates in the daily batch. Browse AI dropped → the credit problem is gone. **One optional nicety still deferred:** in-batch silent-failure visibility (daily `health.py` digest vs in-app Admin badge) — not blocking.
...
- **Reported:** 2026-05-26 · **Type:** Plan/build thread · **Status:** ✅ **RESOLVED** (confirmed 2026-06-02) — code shipped (#590 scraper + frontend fold; #591 host throttle + launchd) AND the **laptop LaunchAgent is installed and running**: `public/bonhams_lots.json` commits ~3×/day on the residential cadence (verified 2026-06-01→06-02).
...
- **Reported:** 2026-05-26 · **Type:** Plan/infra thread · **Status:** ✅ **RESOLVED** (confirmed 2026-06-02) — code shipped (#591: wrapper `scripts/bonhams_residential_scrape.sh`, hourly LaunchAgent plist, setup doc, scraper `--throttle`) AND **installed + running on Mark's laptop** (evidenced by the ~3×/day `bonhams_lots.json` commits). Runs from a dedicated clone on main so it never disturbs the working copy. Future host swap (Pi / Mac mini) is optional, code-unchanged.
...
- **Reported:** 2026-06-02 · **Type:** Filter-state bleed across tabs · **Severity:** 2 · **Surface:** Listings tab after viewing an auction catalog · **Status:** RESOLVED 2026-06-02 (#768). Root cause was `filterSaleUrls` (the single-catalog filter) persisting after you left the auctions/sold context — on Live (or after a tab switch) it filtered the grid by a sale whose lots don't live there → "Nothing matches". Fix: an App effect drops `filterSaleUrls` whenever the user isn't in the auctions/sold catalog context (it never fires mid-open since opening a catalog sets tab+sub in the same pass).
- **Detail:** From an auction catalog, tapping the **Listings/Watches** tab lands on Listings with the **auction sale filter still applied**, so the live grid shows nothing ("Nothing matches · Loosen the filters"). #744 reset-to-base on *return to Auctions* but not on the cross-tab jump *out* to Listings.
