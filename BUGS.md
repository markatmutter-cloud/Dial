...
- **Reported:** 2026-06-13 (Mark — "Christie's still doesn't have the listings to view, just the external link") · **Type:** Scrape coverage gap · **Severity:** 2 · **Surface:** `enumerate_christies` in `auction_lots_scraper.py` · **Status:** Open — needs the enumerator extended. **Detail:** the calendar lists "Watches Online: The New York Edit" but it has **0 scraped lots**, so the catalogue offers only the ↗ external link (no "View lots"). Root cause: Christie's runs its **online-only** sales on a *separate platform/subdomain* — the calendar URL is `https://onlineonly.christies.com/sso?SaleID=30716&SaleNumber=23588`, not the `christies.com/en/auction/<slug>-<id>/` shape the enumerator parses. So the online-only editions are never enumerated (the main live sales like "Rare Watches" / "Important Watches" still work). **Fix direction:** add an `onlineonly.christies.com` branch to `enumerate_christies` (resolve the `sso?SaleID=…` link to the online-only catalog API/page and parse its lot list) — its own investigation: the onlineonly platform's lot payload shape needs mapping, like the Sotheby's GraphQL work (B-69). NB the main Christie's lots currently carry an empty `auction_title` — worth confirming the live-sale → lots linkage during that work.
