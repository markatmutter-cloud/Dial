...
### B-69 — Sotheby's auction lots stopped scraping (catalogue empty; only 1 lot)
- **Reported:** 2026-06-12 (Mark — "Sotheby's shows as a link, lots live, but no listing available") · **Type:** Scrape defect / upstream site change · **Severity:** 2 · **Surface:** `enumerate_sothebys` in `auction_lots_scraper.py` · **Status:** ✅ Fixed (this PR). **Detail:** the calendar still listed Sotheby's sales but the catalogues were empty — `public/auction_lots.json` held **1** Sotheby's lot vs hundreds for Christie's/Phillips. Root cause: Sotheby's stopped server-rendering the `algoliaJson` payload into the auction page (`pageProps.algoliaJson` is now `null`; only the first 48 lots ship inline as a `lotCardsConnection`). The enumerator keyed off `algoliaJson` for both the lot list and the bootstrap-lot slug, so every sale logged `algoliaJson: 0 hits / no bootstrap slug` → `0 lots kept`. **Fix:** rewrote `enumerate_sothebys` to drive the site's own Cosmo GraphQL router (`clientapi.prod.sothelabs.com/graphql`): fetch the auction page for `auctionId` + sale metadata (title/currency/slug/end-date from apolloCache), then paginate the page's `LotCardsFilterByPaginated` query (inlined self-contained so a JS chunk-hash redeploy can't break it) by offset/limit. Each card carries title, lot number, estimate, image (brightspot CDN), and — on closed sales — the realised price (`bidState.sold.ResultVisible.premiums.finalPrice`). One ~5-call walk replaces the old per-lot fetch storm; the now-orphaned `_scrape_sothebys_lot_full` / `scrape_sothebys_lot_image` helpers were removed. **Verified the real path:** live Fine Watches → 414 watch lots (all with estimate + image); closed sale → 160 lots all with realised prices (e.g. 204,800 CHF), currency correctly per-lot. **Trade-off:** lot descriptions/essays aren't in the card payload (description = maker + title, essay fields empty) — that's the card data the feed needs; the per-lot essay fetch was the slow, WAF-risky part, dropped deliberately. Regression test: `tests/test_sothebys_lots.py` (estimate/sold/image/exclusion mapping).

### B-70 — Monaco Legend sold prices ~1000× too small (CHF entity-separator)
- **Reported:** 2026-06-12 (Mark — "Monaco Legend sold figures incorrect; strange price format with a comma, in Swiss francs, but should be much more") · **Type:** Scrape parse defect · **Severity:** 2 · **Surface:** `_mla_parse_sold_price` / `_mla_parse_estimation` in `auction_lots_scraper.py` · **Status:** ✅ Fixed (this PR). **Detail:** the "Exclusive Timepieces 41" sale (CHF) showed sold prices like **$198 / $49 / $137** on the Sold tab — should be tens/hundreds of thousands. The CHF lots render the thousands separator as the HTML numeric entity `&#039;` (apostrophe): the live bid-price span is literally `Fr.\xa032&#039;500`. `_mla_parse_sold_price` normalised a few named entities but never unescaped `&#039;`, so the number regex `\d[\d',. ]*` stopped at the `&` and read only `32` → CHF 32 instead of 32,500 (~1000× low). Estimates were correct (clean source), so only the realised figure was wrong; the prior EUR sale (Exclusive Timepieces 40) used a dot separator and parsed fine, which is why most of the data looked right. **Fix:** `html.unescape()` the text first in both `_mla_parse_sold_price` and `_mla_parse_estimation` (turns `&#039;`→`'`, already in the separator class). **Verified the real path:** re-ran the live sale — 151 sold lots, 0 still under 1000 (CHF 32,500 / 575,000 etc., USD conversion correct). Regression test: `tests/test_monacolegend_price.py` (all three currency formats incl. the entity-separated CHF one).

### B-71 — Non-watch Sotheby's sales appearing in the auction calendar/catalogue
- **Reported:** 2026-06-13 (Mark, real use) · **Type:** Data quality / catalog exclusion · **Severity:** 3 · **Surface:** `EXCLUDE_CATALOG_TITLES` (merge.py calendar + auction_lots_scraper.py lots, lockstep) · **Status:** ✅ Fixed (this PR). **Detail:** two jewels/objets Sotheby's sales were surfacing on the watch site — **"Artistic Luxury: Fabergé, Gold Boxes, Silver & Ceramics"** (167 lots) and **"A Marvelous Journey: The Collection of Maurice Tempelsman"** (127 lots) — both cross-listed in the watches category with a handful of watch lots, the per-lot filter letting them through. **Fix:** added `"Artistic Luxury"` + `"Maurice Tempelsman"` substrings to `EXCLUDE_CATALOG_TITLES` in BOTH lockstep copies; verified `is_excluded_catalog` excludes them while "Fine Watches" / "Important Watches" still pass. Regression test added to `tests/test_merge_state.py`.

### B-72 — Bonhams auction calendar frozen since 2026-04-28 (CI can no longer fetch the department page)
- **Reported:** 2026-06-13 (Mark — "none of the Bonhams auctions are showing"; only "Weekly: Watches") · **Type:** Scrape infrastructure (geo/Cloudflare block) · **Severity:** 2 · **Surface:** `bonhams_auctions_scraper.py` + `scripts/bonhams_residential_scrape.sh` → `data/bonhams_auctions.csv` · **Status:** ✅ Fixed (this PR). **Detail:** `data/bonhams_auctions.csv` last changed **2026-04-28 (7 weeks)** so the calendar is stale; the live fine-watch sales (32648 New York Fine Watches, 31779 Paris Fine Watches, 32629 Luxury Wristwatches at Bruun Rasmussen) are missing. **The scraper code is fine** — run from the residential laptop it emits all 4 current sales correctly (verified 2026-06-13). The cause is environmental: Bonhams' Cloudflare now **403s the department page from GitHub Actions / datacenter IPs**, same block that already forced the Bonhams *lot* scrape onto the residential host (B-24). CLAUDE.md's "Calendar still scrapes in CI" note is now stale. **Fix direction (the established pattern, not a code bug):** move the Bonhams *calendar* scrape onto the laptop `launchd` agent alongside `bonhams_lots_scraper.py`, committing to a residential-owned CSV/JSON that merge.py folds into the calendar (mirrors the lots residential pipeline; `scripts/RESIDENTIAL_SCRAPE_SETUP.md`). Do NOT try to un-block Bonhams in CI — the residential host is the answer. **Fix (this PR):** the laptop wrapper `scripts/bonhams_residential_scrape.sh` now runs the **calendar** scraper too (→ commits `data/bonhams_auctions.csv` alongside the lots); CI's calendar step keeps 403ing → "bonhams missing" → merge.py keeps + re-emits the residential CSV. The wrapper `git pull`s main each tick so this propagates to the laptop's dedicated clone automatically. Also fixed `has_catalog` detection (the `View <span>41</span> lots` count-span split the "View … lots" phrase → False for every published catalog; now strips tags first). Committed a fresh `data/bonhams_auctions.csv` (4 current sales, has_catalog correct) as the immediate stopgap so the sales appear on the next CI merge; docs (RESIDENTIAL_SCRAPE_SETUP.md, bonhams_lots_scraper.py header, CLAUDE.md) corrected. **Follow-up (one):** confirm the laptop agent picked up the new wrapper + that lots populate for the 3 fine-watch sales on the next tick.

### B-73 — Christie's "Watches Online" editions show only the external link (no in-app lots)
- **Reported:** 2026-06-13 (Mark — "Christie's still doesn't have the listings to view, just the external link") · **Type:** Scrape coverage gap · **Severity:** 2 · **Surface:** `enumerate_christies` in `auction_lots_scraper.py` · **Status:** ✅ Fixed (this PR). **Detail:** the calendar lists "Watches Online: The New York Edit" but it had **0 scraped lots**, so the catalogue offers only the ↗ external link (no "View lots"). Root cause: Christie's runs its **online-only** sales on a *separate platform/subdomain* — the calendar URL is `https://onlineonly.christies.com/sso?SaleID=30716&SaleNumber=23588`, not the `christies.com/en/auction/<slug>-<id>/` shape the enumerator parses. So the online-only editions are never enumerated (the main live sales like "Rare Watches" / "Important Watches" still work). **Fix (this PR):** the online-only platform turned out to use the **same data shape** — it just assigns the WHOLE `window.chrComponents = {…,"lots":{data:{…}},…}` as one blob (vs the main site's `window.chrComponents.lots = {…}`). Added `_christies_lots_blob` to read both forms; `enumerate_christies` now detects online-only from the **resolved** URL (the `sso?` link 302s to `onlineonly.christies.com/s/<slug>/lots/<saleid>`), uses that host for lot URLs, paginates the resolved listing URL (`?page=N`), skips the per-lot essay fetch there, and coerces the platform's `"18000.00"` estimate strings to clean ints (`_chr_num`). `auction_url` resolves to the calendar `sso` URL so the catalogue "View lots" matches. **Verified the real path:** NY Edit → 87/87 lots with estimates + images; main-site Rare Watches regression → 225 lots unchanged. Tests: `tests/test_christies_online.py` (both blob forms + `_chr_num`). **Follow-up fix (2026-06-13, the missing half):** #881 taught `enumerate_christies` the online-only payload but the **dispatcher never routed the sale to it** — `ENUMERATORS["Christie's"]` only matched the URL needle `christies.com/en/auction/`, so the `onlineonly.christies.com/sso?…` sale was skipped (0 lots persisted). Added `"onlineonly.christies.com/"` to the needle tuple; **verified through the real `main()` dispatch this time** (route + active-window + enumerate → 87 lots, auction_url == sale.url). Lesson reinforced ([[feedback_test_the_conversation]]): I'd tested `enumerate_christies` directly but not the dispatch path that actually calls it. **Third fix (2026-06-14, the CI-specific half):** even routed, the CI run logged `onlineonly.christies.com … Read timed out` — that host slow-walls CI datacenter IPs (the main `christies.com` works in CI; the online-only host doesn't), the same block that hit watchcenter (B-58). Switched the online-only fetch to `curl_cffi` Chrome-TLS impersonation (`_christies_fetch`), the proven unblock; `www.christies.com` keeps plain requests. Confirm on the next CI run's log that the timeout is gone (residential works, so this can only be verified in CI). **Fourth fix (2026-06-14, images):** even with lots flowing, `onlineonly.christies.com` IP-gates the lot *image* to CI (the blob carries a `christies-roundel.svg` placeholder) — so cards showed "IMAGE NOT AVAILABLE". The real images live on `www.christies.com` (loads from CI) at a deterministic path; `_christies_online_image` reconstructs it from sale number (sso URL) + year + a city→location-code map (`CHRISTIES_LOC_CODES`, New York→NYR) + zero-padded lot number (slug-less form verified 200; 15/15 real lots). Residential runs keep the embedded real URL; CI reconstructs.

### B-74 — DKK auction prices shown unconverted (DKK 60,000 labelled "$60,000")
- **Reported:** 2026-06-14 (Mark, Bonhams Bruun Rasmussen / Copenhagen sale) · **Type:** Currency / FX gap · **Severity:** 2 · **Surface:** FX tables (`merge.py` `FX` + `src/utils.js` `FX_RATES_USD_PER`, lockstep) · **Status:** ✅ Fixed (this PR). **Detail:** the Bruun Rasmussen lots are priced in **DKK**, but cards showed e.g. "CURRENT BID **$60,000** · DKK 60,000" — the USD figure was the raw DKK amount because **DKK wasn't in the FX table**, so `FX_RATES_USD_PER["DKK"]` was `undefined` and the conversion fell back to ×1 (`Card.js:194`). **Fix:** added `DKK: 0.145` (≈ 1 USD = 6.9 DKK, EUR-peg-derived) to **both** lockstep FX tables + `DKK: "DKK "` to `CURRENCY_SYM`. Now DKK 60,000 → ~$8,700. FX-parity guard green; `to_usd(60000,"DKK")==8700` verified. Frontend-only display fix (the card computes USD live from the native value), so it takes effect on the next bundle deploy — no re-scrape needed.

### B-75 — Per-house auction health check (catch silent-zero enumerator breaks)
- **Reported:** 2026-06-14 (Mark — "why are there so many errors on the auction side"; built same session) · **Type:** Observability / platform health · **Severity:** 2 · **Surface:** `auction_health.py` + `scrape-auction-lots-frequent.yml` · **Status:** ✅ Built (this PR). **Detail:** the dealer scrape-health gate (B-66) catches a *dealer* CSV going missing, but the **auction** side had no equivalent — so when a house's enumerator broke and its lots quietly fell to ~zero while the calendar still listed the sale, it stayed invisible until Mark spotted it (Sotheby's GraphQL migration → 1 lot; Christie's online-only → 0 lots; Bonhams calendar 403 → no sales). **Built:** `auction_health.py` flags any house with a **published, current catalog** (`auctions.json` hasCatalog + a near-term date, within `[today-3, today+45]`) but **0 lots** in the merged feed (`auction_lots.json`; Bonhams counted from `bonhams_lots.json`). Debounced exactly like B-66 — `--record` (before commit) bumps per-house consecutive-empty counts in `data/auction_health_state.json`, `--check` (final, always()) reds the job at `THRESHOLD=3` so notify-scrape-failure opens an issue. Verified against live data (all 8 houses healthy, 0 broken); 8 unit tests (broken/healthy/Bonhams-feed/no-catalog/past/far-future/debounce/threshold). **Follow-up (one):** also wire `--check` into `scrape-auctions.yml` (calendar) so a house with **no calendar sales at all** (the Bonhams-403 mode) is caught there too — this PR covers the lots producer.
