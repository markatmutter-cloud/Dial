...
- **Mark's decision (2026-05-26):** don't fight the 50-credit free cap — **subscribe** and make it worthwhile by adding Browse-AI-only sources (Bonhams etc. → B-24), then **move to self-hosted Playwright sooner** (→ B-25). Until credits are topped up, the dedicated fresh-trigger workflow will keep 403-ing (~2×/day email).
- **Remaining:** (a) **Mark:** add Browse AI credits (subscribe/top-up); the 403 clears once balance covers a fresh run. (b) **Interim noise:** optionally pause `scrape-tropicalwatch.yml`'s `schedule:` (keep `workflow_dispatch`) so it stops the guaranteed-fail emails — TW still flows via the main batch's `--latest`. (c) **Decide:** in-batch silent-failure visibility (daily `health.py` digest vs loud-batch vs in-app Admin badge) — deferred. Premise note: the original "3 Browse AI dealers" read was wrong — only Tropical Watch uses it.

### B-24 — Browse AI source expansion: Bonhams + other Cloudflare-blocked houses (plan thread)
- **Reported:** 2026-05-26 · **Type:** Plan/product thread, not a defect · **Status:** Open — flagged for plan-mode. Greenlit in principle by Mark as the justification for a Browse AI **subscription**.
- **Detail:** Several sources can't be scraped from CI because GitHub runners hit Cloudflare IP-reputation walls — **Bonhams** is the named one (CLAUDE.md: "blocked from CI by Cloudflare IP-reputation — works locally; don't burn time on client-side disguise"). Browse AI runs real browser sessions that clear those walls, so it's the natural tool for exactly this class. **Scoping needed:** which houses/dealers to add, a credit budget per robot (each fresh capture ≈ rows/10 credits), build one Browse AI robot per site + a per-source scraper (per-source-file convention), and confirm the subscription tier covers the total credit load. Trades against the same budget as TW — size it before building.

### B-25 — Self-hosted Playwright runner (spare laptop) for blocked sources (plan thread)
- **Reported:** 2026-05-26 · **Type:** Plan/infra thread, not a defect · **Status:** Open — flagged for plan-mode. Mark wants to move here "sooner" as the long-term, zero-Browse-AI-cost path.
- **Detail:** Stand up a **self-hosted GitHub Actions runner** on Mark's spare Mac and run the Cloudflare/bot-blocked scrapers (Bonhams, and anything else CI can't reach) there via **Playwright** — from a residential IP where they already work locally, at no Browse AI credit cost. **Scoping needed:** runner setup + security (self-hosted runners on a public repo are a known risk — restrict to private workflows or a dedicated minimal-permission runner), which scrapers move to it, keep-alive/uptime of the laptop, and how it coexists with the hosted scrape workflows. The eventual replacement for paid Browse AI on the blocked sources.
